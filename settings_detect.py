...
RUN_ID = "LINEARSVC_3_CBEVENT{}_{}".format(TIMESTAMP, LANGUAGE)
DATASET_NAME = "CBEVENT_{}".format(LANGUAGE.upper())
...
MULTICLASS = False  # set to False for binary classification and true for multiclass
HOLDOUT = 0.1
LABELMAP = dict(((-1, "no_bullying"), (1, "bullying")))
    ...
    "en": VotingClassifier(
        estimators=[
            (
                "logres",
                LogisticRegression(
                    C=0.02, class_weight="balanced", random_state=RANDOM_STATE
                ),
            ),
            (
                "linearsvc",
            ...
            (
                "sgd",
                SGDClassifier(
                    class_weight=None, n_iter=9.0, random_state=RANDOM_STATE,
                ),
            ),
        ],
        voting="soft",
...
randomus_params = {"ratio": ["auto", 0.5]}
onesidedsel_params = {}
...
PIPE_STEPS = [
    ...
    (
        "classify",
        [
            # [(MaxAbsScaler(), scaler_params), (LinearSVC(random_state=RANDOM_STATE), linearsvc_params)],
            [
                (MaxAbsScaler(), scaler_params),
                (LinearSVC(random_state=RANDOM_STATE), linearsvc_params),
            ],  # PRETTY FAST
            # (NuSVC(random_state=RANDOM_STATE), nusvc_params), # TOO SLOW
            # (LogisticRegression(random_state=RANDOM_STATE), logregr_params), # FAST
            # (SGDClassifier(random_state=RANDOM_STATE), sgd_params), # FAST
            # (PassiveAggressiveClassifier(random_state=RANDOM_STATE), passaggr_params), # FAST
            ...
            # # VOTING CLF
            # (votingclf[LANGUAGE], voting_params),
        ],
    ),
]
...
SCORER_METRIC = "recall"
SCORE_AVERAGING = "binary"
